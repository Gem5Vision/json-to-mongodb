...
folder_path = 'gem5/configs/example'
folder_path_2= 'gem5/tests/gem5'
files = []
...
print(files)
# Search for each file in folder_path_2
for file in files:
    for root, directories, filenames in os.walk(folder_path_2):
        for filename in filenames:
            if filename.endswith('.py'):
                with open(os.path.join(root, filename), 'r') as f:
                    if file in f.read():
                        print(f'"{file}" found in {os.path.join(root, filename)}')
